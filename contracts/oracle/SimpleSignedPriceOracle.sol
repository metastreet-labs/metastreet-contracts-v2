...
contract SimpleSignedPriceOracle is Ownable2Step, EIP712, IPriceOracle {
    ...
    /**
     * @notice Quote type
     * @dev QuoteV1 is used for single token ID
     * @dev QuoteV2 is used for a given range of token IDs
     */
    enum QuoteType {
        QuoteV1,
        QuoteV2
    ...
    /**
     * @notice Verify multiples quote and signer
     * @param collateralToken Collateral token
     ...
     * @return Total average oracle price
     */
    ...
    function price(
        ...
        /* Get price oracle signer */
        PriceOracleSigner memory priceOracleSigner_ = _priceOracleSigners[collateralToken];

        /* Validate price oracle signer is set */
        if (priceOracleSigner_.signer == address(0)) revert InvalidSigner();

        return
            priceOracleSigner_.quoteType == QuoteType.QuoteV1
                ? _verifyQuoteV1(
                    collateralToken,
                    collateralTokenIds,
