...
contract TestDN404Mirror is DN404Mirror {
    ...
    function tokenURI(uint256 id) public view virtual override brutalizeMemory returns (string memory result) {
    ...
    function balanceOf(address owner) public view virtual override brutalizeMemory returns (uint256 result) {
        result = DN404Mirror.balanceOf(_brutalized(owner));
    }

    function ownerOf(uint256 id) public view virtual override brutalizeMemory returns (address result) {
    ...
    function getApproved(uint256 id) public view virtual override brutalizeMemory returns (address result) {
        result = DN404Mirror.getApproved(id);
    }

    function setApprovalForAll(address operator, bool approved) public virtual override brutalizeMemory {
    ...
    function transferFrom(address from, address to, uint256 id) public virtual override brutalizeMemory {
