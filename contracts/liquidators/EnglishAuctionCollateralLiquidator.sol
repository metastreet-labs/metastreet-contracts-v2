...
contract EnglishAuctionCollateralLiquidator is ICollateralLiquidator, ReentrancyGuard {
     ...
     */
    uint64 private _incrementPercentage;
    ...
    function bid(bytes32 collateralHash, uint256 amount) external nonReentrant {
        ...
        if (
            amount <=
            Math.mulDiv(auction_.highestBid.amount, BASIS_POINTS_SCALE + _incrementPercentage, BASIS_POINTS_SCALE)
        ) revert InvalidBid();
