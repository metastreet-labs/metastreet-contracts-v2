...
/**
 * @title English Auction Collateral Liquidator (trusted)
...
contract EnglishAuctionCollateralLiquidator is ICollateralLiquidator, ReentrancyGuard {
     ...
     * @param auctionCount Number of auctions that have not ended with a winning bid
    ...
    /**
     * @notice Emitted when collateral auction is created
    ...
    /**
     * @notice Emitted when collateral auction is started
     * @param collateralHash Collateral hash
     * @param collateralToken Collateral token
     * @param collateralTokenId Collateral token ID
     */
     ...
     */
    event CollateralBidded(bytes32 indexed collateralHash, address indexed bidder, uint256 amount);

    /**
     * @notice Emitted when collateral is claimed by auction winner
     * @param bidder Winner of auction
     * @param collateralToken Collateral token
     * @param collateralTokenId Collateral token ID
     ...
     * @param collateralToken Collateral token
     * @param isCollateralWrapper True if token is collateral wrapper, otherwise false
     */
    event CollateralWrappersUpdated(address collateralToken, bool isCollateralWrapper);
    ...
    /**
     * @notice Helper function to process colalteral liquidation
     *
    ...
    function _processLiquidation(
        ...
        /* Update liquidation incompleted auctions */
    ...
    /**
     * @notice Bid collateral
     *
     * Emits a {CollateralBidded} event.
     *
     * @param collateralHash Collateral hash
     * @param amount Bid amount
     */
    function bid(bytes32 collateralHash, uint256 amount) external nonReentrant {
        ...
        if (auction_.highestBid.bidder != address(0)) {
            /* Transfer bid from collateral liquidator to previous bidder */
        ...
        /* Emit CollateralBidded */
        emit CollateralBidded(collateralHash, msg.sender, amount);
    }
     ...
     *
     * Emits a {CollateralLiquidated} event.
     * Emits a {CollateralClaimed} event.
     *
     * @param collateralHash Collateral hash
    ...
    function claim(
        ...
        /* Validate collateral context and liquidation Context */
    ...
    /**
     * @notice Update collateral wrappers
     *
     * Emits a {CollateralWrappersUpdated} event.
     *
     * @param collateralToken Collateral token
     * @param isCollateralWrapper True if address is a collateral wrapper, otherwise false
     */
    function setCollateralWrapper(address collateralToken, bool isCollateralWrapper) external {
        if (msg.sender != _admin) revert InvalidCaller();

        /* Update collateral wrappers */
        _collateralWrappers[collateralToken] = isCollateralWrapper;

        /* Emit CollateralWrappersUpdated */
        emit CollateralWrappersUpdated(collateralToken, isCollateralWrapper);
    }
}
