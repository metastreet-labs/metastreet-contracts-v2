...
library LoanReceipt {
    ...
    */
    ...
    function decode(bytes calldata encodedReceipt) internal pure returns (LoanReceiptV1 memory) {
        ...
        /* Validate encoded receipt length */
        if (
            receipt.collateralContextLength != receipt.collateralContextData.length ||
            (receipt.collateralContextLength > 0 && (receipt.collateralContextData.length - 20) % 32 != 0) ||
            ((encodedReceipt.length - LOAN_RECEIPT_V1_HEADER_SIZE - receipt.collateralContextLength) %
