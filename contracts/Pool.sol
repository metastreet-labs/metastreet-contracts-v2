...
contract Pool is ERC165, ERC721Holder, AccessControl, Pausable, ReentrancyGuard, Multicall, IPool, ILiquidity {
    ...
    /**
     * @notice Bundle collateral wrapper token
     */
    ICollateralWrapper internal _bundleCollateralWrapper;
    ...
    function initialize(
        ...
        ICollateralWrapper bundleCollateralWrapper_,
        ...
        _bundleCollateralWrapper = bundleCollateralWrapper_;
    ...
    function bundleCollateralWrapper() external view returns (address) {
        return address(_bundleCollateralWrapper);
    }
    ...
    function _toAssetInfo(
        ...
        if (collateralToken_ == address(_bundleCollateralWrapper)) {
            if ((collateralContextData.length - 20) % 32 != 0) revert InvalidBorrowOptionsEncoding();
            assets = _bundleCollateralWrapper.enumerate(collateralTokenId, collateralContextData);
        } else {
            if (collateralContextData.length > 0) revert InvalidBorrowOptionsEncoding();
    ...
    function borrow(
        ...
        if (collateralToken_ != address(_bundleCollateralWrapper) && options.length > 0) {
