    ...
    ICollateralLiquidationReceiver
{
    ...
    function onCollateralLiquidated(bytes calldata encodedLoanReceipt, uint256 proceeds) external nonReentrant {
        ...
        /* Compute proceeds less repayment */
        uint256 bonus = proceeds > loanReceipt.repayment ? proceeds - loanReceipt.repayment : 0;

        /* Restore liquidity nodes */
        uint128 proceedsRemaining = (proceeds - bonus).toUint128();
        for (uint256 i; i < loanReceipt.nodeReceipts.length; i++) {
            /* Restore node */
        ...
        /* Transfer bonus to borrower */
        if (bonus != 0) IERC20(_currencyToken).transfer(loanReceipt.borrower, bonus);
