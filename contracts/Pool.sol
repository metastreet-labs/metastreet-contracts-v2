    ...
    ICollateralLiquidationReceiver
{
    ...
    function depositSharePrice(uint128 tick) external view returns (uint256) {
        return _unscale(_storage.liquidity.depositSharePrice(tick), true);
    }
    ...
    /**
     * @dev Helper function to scake down a value
     * @param value Value
    ...
    function _unscale(uint256 value, bool isRoundUp) internal view returns (uint256) {
        ...
        return !isRoundUp || value % factor == 0 ? value / factor : value / factor + 1;
    }
    ...
    function onCollateralLiquidated(bytes calldata encodedLoanReceipt, uint256 proceeds) external nonReentrant {
        ...
        uint256 unscaledBorrowerSurplus = _unscale(borrowerSurplus, true);
    ...
    function redemptionAvailable(
        ...
        return (shares, _unscale(amount, true), sharesAhead);
    }
    ...
    function rebalance(
        ...
        uint256 unscaledAmount = _unscale(amount, true);
