...
describe("Pool Gas", function () {
  ...
  describe("#merkle", async function () {
    ...
    for (const [count, principal, numTicks, maxGas] of [
      [10, FixedPoint.from("15"), 10, 276500],
      [10, FixedPoint.from("25"), 16, 355700],
      [100, FixedPoint.from("15"), 10, 280100],
      [100, FixedPoint.from("25"), 16, 359300],
      [1000, FixedPoint.from("15"), 10, 284700],
      [1000, FixedPoint.from("25"), 16, 363900],
    ]) {
      it(`merkle borrow (single, ${numTicks} ticks, ${count} token ids)`, async function () {
        ...
        proxy = await testProxyFactory.deploy(
          ...
          poolImpl.interface.encodeFunctionData("initialize", [
            ethers.utils.defaultAbiCoder.encode(
              ["address", "address", "uint64[]", "uint64[]", "bytes32", "uint32"],
              [
                nft1.address,
                tok1.address,
                [30 * 86400, 14 * 86400, 7 * 86400],
