...
describe("Pool Basic - 404", function () {
  ...
  let dn404MirrorNFT: TestDN404Mirror;
  let dn404: TestDN404;
  ...
  before("deploy fixture", async () => {
    ...
    const testDN404MirrorFactory = await ethers.getContractFactory("TestDN404Mirror");
    const dn404Factory = await ethers.getContractFactory("TestDN404");

    /* Deploy DN404Mirror */
    dn404MirrorNFT = (await testDN404MirrorFactory.deploy(accounts[8].address)) as TestDN404Mirror;

    /* Deploy and and initialize DN404 */
    dn404 = (await dn404Factory.deploy()) as TestDN404;
    await dn404
      .connect(accounts[8])
      .initializeDN404(ethers.utils.parseEther("10"), accounts[8].address, dn404MirrorNFT.address);
    ...
    /* Transfer tokens to borrower, minting NFT */
    await dn404.connect(accounts[8]).transfer(accountBorrower.address, ethers.utils.parseEther("3"));
    ...
    await dn404MirrorNFT.connect(accountBorrower).setApprovalForAll(pool.address, true);
  ...
  describe("constants", async function () {
    ...
    it("matches expected implementation version", async function () {
      expect(await pool.IMPLEMENTATION_VERSION()).to.equal("2.6");
    });
  });
  ...
  describe("#redemptionAvailable", async function () {
    ...
    it("returns written down redemption available from liquidated loan", async function () {
      ...
      await collateralLiquidator
        ...
        .withdrawCollateral(pool.address, tok1.address, dn404MirrorNFT.address, 1, "0x", loanReceipt);
  ...
  async function createActiveLoan(
    ...
    const tokenId =
      (await dn404MirrorNFT.ownerOf(1)) === accountBorrower.address
        ? 1
        : (await dn404MirrorNFT.ownerOf(2)) === accountBorrower.address
        ? 2
        : 3;

    const ticks = await sourceLiquidity(principal);

    const repayment = await pool.quote(principal, duration, dn404MirrorNFT.address, tokenId, ticks, "0x");

    const borrowTx = await pool
      .connect(accountBorrower)
      .borrow(principal, duration, dn404MirrorNFT.address, tokenId, repayment, ticks, "0x");
    const loanReceipt = (await extractEvent(borrowTx, pool, "LoanOriginated")).args.loanReceipt;
  ...
  describe("#quote", async function () {
    ...
    it("quotes repayment from various duration and rate ticks", async function () {
      ...
      expect(await pool.quote(FixedPoint.from("25"), 7 * 86400, dn404MirrorNFT.address, 1, ticks, "0x")).to.equal(
    ...
    it("fails with non-increasing tick", async function () {
      ...
      await expect(
        pool.quote(FixedPoint.from("25"), 7 * 86400, dn404MirrorNFT.address, 1, ticks, "0x")
      ).to.be.revertedWithCustomError(pool, "InvalidTick");
    });
    ...
    it("fails with duplicate ticks", async function () {
      ...
      await expect(
        pool.quote(FixedPoint.from("35"), 7 * 86400, dn404MirrorNFT.address, 1, ticks, "0x")
      ).to.be.revertedWithCustomError(pool, "InvalidTick");
    });
    ...
    it("fails on low duration ticks", async function () {
      ...
      await expect(
        pool.quote(FixedPoint.from("35"), 8 * 86400, dn404MirrorNFT.address, 1, ticks, "0x")
      ).to.be.revertedWithCustomError(pool, "InvalidTick");
    });
  });
  ...
  describe("#borrow", async function () {
    ...
    it("originates loan", async function () {
      ...
      // const r = await borrowTx.wait();
      // console.log(r);

      /* Validate return value from borrow() */
      expect(simulatedRepayment).to.equal(repayment);

      /* Valide token transfer of event */
      await expectEvent(borrowTx, dn404, "Transfer", {
      ...
      /* Validate events */
      await expectEvent(borrowTx, dn404MirrorNFT, "Transfer", {
        from: accountBorrower.address,
        to: pool.address,
        id: 1,
      });
      ...
      expect(decodedLoanReceipt.collateralToken).to.equal(dn404MirrorNFT.address);
      expect(decodedLoanReceipt.collateralTokenId).to.equal(1);
    ...
    it("originates a loan from various duration and rate ticks", async function () {
      ...
      await pool
        .connect(accountBorrower)
        .borrow(FixedPoint.from("25"), 7 * 86400, dn404MirrorNFT.address, 1, FixedPoint.from("26"), ticks, "0x");
    });
    ...
    it("originates loan with v1 delegation", async function () {
      ...
      await expectEvent(borrowTx, delegateRegistryV1, "DelegateForToken", {
        ...
        contract_: dn404MirrorNFT.address,
        tokenId: 1,
        value: true,
      });
      ...
      /* Validate delegation */
      expect(
        await delegateRegistryV1.checkDelegateForToken(accountBorrower.address, pool.address, dn404MirrorNFT.address, 1)
      ).to.equal(true);
    });
    ...
    it("fails with non-increasing tick", async function () {
      ...
      await expect(
        pool
          .connect(accountBorrower)
          .borrow(FixedPoint.from("25"), 30 * 86400, dn404MirrorNFT.address, 1, FixedPoint.from("26"), ticks, "0x")
      ).to.be.revertedWithCustomError(pool, "InvalidTick");
    });
    ...
    it("fails with low duration ticks", async function () {
      ...
      await expect(
        pool
          .connect(accountBorrower)
          .borrow(FixedPoint.from("25"), 8 * 86400, dn404MirrorNFT.address, 1, FixedPoint.from("26"), ticks, "0x")
      ).to.be.revertedWithCustomError(pool, "InvalidTick");
    });
  ...
  describe("#repay", async function () {
    ...
    it("repays loan at maturity", async function () {
      ...
      await expectEvent(repayTx, dn404MirrorNFT, "Transfer", {
        from: pool.address,
        to: accountBorrower.address,
        id: 1,
      });
    ...
    it("repays removes v1 delegation", async function () {
      ...
      expect(await pool.loans(loanReceiptHash)).to.equal(2);
      expect(
        await delegateRegistryV1.checkDelegateForToken(accountBorrower.address, pool.address, dn404MirrorNFT.address, 2)
      ).to.equal(false);
    });
    ...
    it("fails on same block repayment", async function () {
      ...
      nodeReceipt.collateralToken = dn404MirrorNFT.address;
  ...
  describe("#refinance", async function () {
    ...
    it("refinance loan at maturity with admin fee and same principal", async function () {
      ...
      expect(decodedNewLoanReceipt.collateralToken).to.equal(dn404MirrorNFT.address);
      expect(decodedNewLoanReceipt.collateralTokenId).to.equal(1);
  ...
  describe("#liquidate", async function () {
    ...
    it("liquidates expired loan", async function () {
      ...
      /* Validate events */
      await expectEvent(liquidateTx, dn404MirrorNFT, "Transfer", {
        from: pool.address,
  ...
  describe("#onCollateralLiquidated", async function () {
    ...
    it("processes liquidated loan for lower proceeds", async function () {
      ...
      await collateralLiquidator
        .connect(accountLiquidator)
        .liquidateCollateral(pool.address, tok1.address, dn404MirrorNFT.address, 1, "0x", loanReceipt, proceeds);
