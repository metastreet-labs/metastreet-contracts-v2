...
describe("WeightedInterestRateModel", function () {
  ...
  describe("#price", async function () {
    ...
    it("prices interest to four nodes", async function () {
      ...
      const interest = nodes4.reduce(
        (acc, n) =>
          acc.add(
            n.used
              .mul(RATES[Tick.decode(n.tick).rateIndex])
              .mul(30 * 86400)
              .div(ethers.constants.WeiPerEther)
          ),
      ...
      expect(pending[0]).to.equal(nodes4[0].used.add(FixedPoint.from("0.002533782213967835")));
      expect(pending[1]).to.equal(nodes4[1].used.add(FixedPoint.from("0.050675644279356660")));
      expect(pending[2]).to.equal(nodes4[2].used.add(FixedPoint.from("0.129804324682933386")));
      expect(pending[3]).to.equal(nodes4[3].used.add(FixedPoint.from("0.063561591278366119")));
    });
    it("prices interest to four nodes with admin fee", async function () {
      ...
      expect(pending.length).to.equal(4);
      expect(pending[0]).to.equal(nodes4[0].used.add(FixedPoint.from("0.002407093103269444")));
      expect(pending[1]).to.equal(nodes4[1].used.add(FixedPoint.from("0.048141862065388827")));
      expect(pending[2]).to.equal(nodes4[2].used.add(FixedPoint.from("0.123314108448786716")));
      expect(pending[3]).to.equal(nodes4[3].used.add(FixedPoint.from("0.060383511714447813")));
    });
    it("prices interest to five nodes with one dust node", async function () {
      ...
      expect(pending[0]).to.equal(nodes5[0].used.add(FixedPoint.from("0.002533782213967835")));
      expect(pending[1]).to.equal(nodes5[1].used.add(FixedPoint.from("0.050675644279356660")));
      expect(pending[2]).to.equal(nodes5[2].used.add(FixedPoint.from("0.129804324682933386")));
      expect(pending[3]).to.equal(nodes5[3].used);
      expect(pending[4]).to.equal(nodes5[4].used.add(FixedPoint.from("0.063561591278366119")));
    });
    it("prices interest to six nodes with five small deposit nodes", async function () {
      ...
      expect(pending[0]).to.equal(nodes6[0].used.add(FixedPoint.from("0.410943114799472367")));
      expect(pending[1]).to.equal(nodes6[1].used.add(FixedPoint.from("0.000003979698177924")));
      expect(pending[2]).to.equal(nodes6[2].used.add(FixedPoint.from("0.000003979736717786")));
      expect(pending[3]).to.equal(nodes6[3].used.add(FixedPoint.from("0.000003979775257648")));
      expect(pending[4]).to.equal(nodes6[4].used.add(FixedPoint.from("0.000003979813797509")));
      expect(pending[5]).to.equal(nodes6[5].used.add(FixedPoint.from("0.000003979852337371")));
    });
    ...
    it("prices interest to four nodes with admin fee and zero principal", async function () {
      ...
      expect(repayment).to.equal(ethers.constants.Zero);
      expect(adminFee).to.equal(ethers.constants.Zero);
