...
describe("Pool Node Pass Collection", function () {
  ...
  before("fork mainnet and deploy fixture", async function () {
    ...
    const poolImplFactory = await getContractFactoryWithLibraries("WeightedRateNodePassCollectionPool", [
      "LiquidityLogic",
      "DepositLogic",
      "BorrowLogic",
    ...
    /* Deploy test NFT */
    nft1 = (await ethers.getContractAt("TestERC721", NODE_PASS)) as TestERC721;
    ...
    minter = await ethers.getImpersonatedSigner(YIELD_PASS_FACTORY);
    accountLender = accounts[5];
    accountLiquidator = accounts[6];

    /* Set balance for borrower and minter */
    await network.provider.send("hardhat_setBalance", [
      await accountBorrower.getAddress(),
      ethers.toBeHex(ethers.parseEther("100.0")), // 100 ETH
    ]);
    await network.provider.send("hardhat_setBalance", [
    ...
    /* Mint node pass to borrower (masquerading as yield pass factory) */
    const INodePass = ["function mint(address to, uint256[] calldata tokenIds) external"];
    const nft1_ = await ethers.getContractAt(INodePass, NODE_PASS);
    await nft1_.connect(minter).mint(await accountBorrower.getAddress(), [67291]);
  ...
  describe("constants", async function () {
    it("matches expected implementation name", async function () {
      expect(await pool.IMPLEMENTATION_NAME()).to.equal("WeightedRateNodePassCollectionPool");
    });
  });
  ...
  describe("#quote", async function () {
    ...
    it("correctly quotes repayment with yield pass collection filter", async function () {
      expect(
        await pool.quote(
          FixedPoint.from("10"),
          30 * 86400,
    ...
    it("fails on unsupported collateral with yield pass collection filter", async function () {
      await expect(
        pool.quote(
          FixedPoint.from("10"),
          30 * 86400,
  ...
  describe("#borrow", async function () {
    ...
    it("originates loan with yield pass collection filter", async function () {
      /* Quote repayment */
      ...
      expect(decodedLoanReceipt.collateralTokenId).to.equal(67291);
  ...
  describe("#refinance", async function () {
    ...
    it("refinance loan at maturity with same principal and yield pass collection filter", async function () {
      const borrowTx = await pool
        .connect(accountBorrower)
        .borrow(
          FixedPoint.from("25"),
      ...
      expect(decodedNewLoanReceipt.collateralTokenId).to.equal(67291);
