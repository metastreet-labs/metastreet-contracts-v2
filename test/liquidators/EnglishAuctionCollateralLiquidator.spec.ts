...
describe("EnglishAuctionCollateralLiquidator", function () {
  ...
  describe("#start auction", async function () {
    it("succeeds calling liquidate on collateral", async function () {
      ...
      await expect(auction.startTime).to.equal(0);
    ...
    it("succeeds starting an auction on bundled collateral", async function () {
      /* Construct loan reciept */
      const contextData = ethers.utils.solidityPack(["address", "uint256[]"], [nft1.address, [123, 124, 125]]);
      ...
      const loanReceipt = await loanReceiptLibrary.encode(
        makeLoanReceipt(
          bundleCollateralWrapper.address,
          bundleTokenId,
          hexToBytes(contextData.slice(2)).length,
          contextData
        )
      );
    ...
    it("succeeds starting an auction on non-approved bundled collateral", async function () {
      /* Construct loan reciept */
      const contextData = ethers.utils.solidityPack(["address", "uint256[]"], [nft1.address, [126, 127, 128]]);
    ...
    it("failed liquidate if there exists the same liquidation hash", async function () {
      /* Construct loan reciept */
      const loanReceipt = await loanReceiptLibrary.encode(makeLoanReceipt(nft1.address, 122, 0, "0x"));
    ...
    it("failed liquidate with invalid token", async function () {
      /* Construct loan reciept */
  ...
  describe("#bid", async function () {
    it("3 successful bids on same auction", async function () {
      ...
      await expectEvent(bid1Tx, collateralLiquidator, "CollateralBidded", {
        collateralHash: collateralHash,
      ...
      /* Validate state */
      const transactionTime = await getTransactionTimestamp(bid1Tx.blockNumber);
      ...
      await expectEvent(bid2Tx, collateralLiquidator, "CollateralBidded", {
        collateralHash: collateralHash,
      ...
      await expectEvent(bid3Tx, collateralLiquidator, "CollateralBidded", {
        collateralHash: collateralHash,
        bidder: accountBidder1.address,
    ...
    it("successful time extension within 10 minutes of end time", async function () {
      /* Construct loan reciept */
      const loanReceipt = await loanReceiptLibrary.encode(makeLoanReceipt(nft1.address, 122, 0, "0x"));
      ...
      const transactionTime = await getTransactionTimestamp(bid1Tx.blockNumber);
      elapseUntilTimestamp(transactionTime + 86400 - 10 * 60);
      ...
      /* Validate state */
      const bid2TransactionTime = await getTransactionTimestamp(bid2Tx.blockNumber);
      let auction = await collateralLiquidator.auction(collateralHash);
      await expect(auction.startTime).to.equal(transactionTime);
      await expect(auction.endTime).to.equal(bid2TransactionTime + 60 * 20);
    });

    it("failed when bid with 0 amount", async function () {
      /* Construct loan reciept */
      const loanReceipt = await loanReceiptLibrary.encode(makeLoanReceipt(nft1.address, 122, 0, "0x"));
    ...
    it("failed when bid after end time", async function () {
      /* Construct loan reciept */
      const loanReceipt = await loanReceiptLibrary.encode(makeLoanReceipt(nft1.address, 122, 0, "0x"));
      ...
      const transactionTime = await getTransactionTimestamp(bid1Tx.blockNumber);
      elapseUntilTimestamp(transactionTime + 86400 + 1);
    ...
    it("failed when bid with same amount as previous bid", async function () {
      /* Construct loan reciept */
      const loanReceipt = await loanReceiptLibrary.encode(makeLoanReceipt(nft1.address, 122, 0, "0x"));
    ...
    it("failed when bid with amount smaller than previous bid", async function () {
      /* Construct loan reciept */
      const loanReceipt = await loanReceiptLibrary.encode(makeLoanReceipt(nft1.address, 122, 0, "0x"));
  ...
  describe("#claim", async function () {
    it("successful claim single collateral", async function () {
      /* Construct loan reciept */
      const loanReceipt = await loanReceiptLibrary.encode(makeLoanReceipt(nft1.address, 122, 0, "0x"));
      ...
      /* Fast forward to 1 second after end time */
      const transactionTime = await getTransactionTimestamp(bidTx.blockNumber);
      elapseUntilTimestamp(transactionTime + 86400 + 1);
      ...
      await expect(claimTx)
        .to.emit(collateralLiquidator, "CollateralClaimed")
    ...
    it("successful claim bundled collateral", async function () {
      /* Construct loan reciept */
      const contextData = ethers.utils.solidityPack(["address", "uint256[]"], [nft1.address, [123, 124, 125]]);
      ...
      const claim1Tx = await collateralLiquidator
        ...
        .claim(
          ...
          bundleCollateralWrapper.address,
          bundleTokenId,
          hexToBytes(contextData.slice(2)),
          loanReceipt
        );
    ...
    it("Successful claim as non-winner", async function () {
      /* Construct loan reciept */
      const loanReceipt = await loanReceiptLibrary.encode(makeLoanReceipt(nft1.address, 122, 0, "0x"));
    ...
    it("successful claim of collateral liquidated by EOA", async function () {
      /* Construct loan reciept */
    ...
    it("failed claim before end time", async function () {
      /* Construct loan reciept */
      const loanReceipt = await loanReceiptLibrary.encode(makeLoanReceipt(nft1.address, 122, 0, "0x"));
    ...
    it("failed claim after claiming successful before", async function () {
      /* Construct loan reciept */
      const loanReceipt = await loanReceiptLibrary.encode(makeLoanReceipt(nft1.address, 122, 0, "0x"));
