...
describe("Pool Basic Grace Period", function () {
  ...
  describe("#deposit", async function () {
    it("deposits reverts if not whitelisted", async function () {
      await expect(pool.connect(accountDepositors[0]).deposit(Tick.encode("10"), FixedPoint.from("25"), 0)).to.be.revertedWithCustomError(pool, "InvalidCaller");
    });
  });
  ...
  describe("#setDepositWhitelist", async function () {
    it("admin set deposit whitelist successfully", async function () {
      ...
      /* Validate rate was successfully set */
      expect(await pool.isDepositWhitelisted(accountDepositors[0], 1)).to.equal(true);
      expect(await pool.isDepositWhitelisted(accountDepositors[1], 1)).to.equal(false);
    });
  });
});
