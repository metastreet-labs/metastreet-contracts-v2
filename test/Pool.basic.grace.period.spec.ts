...
import {
  ...
  WeightedRateCollectionPoolWithGracePeriod,
} from "../typechain";
...
describe("Pool Basic Grace Period", function () {
  ...
  before("deploy fixture", async () => {
    ...
    const poolImplFactory = await getContractFactoryWithLibraries("WeightedRateCollectionPoolWithGracePeriod", [
      "LiquidityLogic",
      "DepositLogic",
      "BorrowLogic",
    ...
    proxy = await testProxyFactory.deploy(
      ...
      poolImpl.interface.encodeFunctionData("initialize", [
        ethers.AbiCoder.defaultAbiCoder().encode(
          ["address[]", "address", "address", "uint64[]", "uint64[]", "uint256", "uint256"],
          [
  ...
  describe("constants", async function () {
    it("matches expected implementation name", async function () {
      expect(await pool.IMPLEMENTATION_NAME()).to.equal("WeightedRateCollectionPoolWithGracePeriod");
    });
  ...
  describe("#repay", async function () {
    ...
    it("repay after grace period", async function () {
      const [loanReceipt, loanReceiptHash] = await createActiveLoan(FixedPoint.from("25"));
      ...
      const totalInterest = baseInterest + gracePeriodInterest;

      /* Validate events */
      await expectEvent(repayTx, pool, "LoanRepaid", {
        loanReceiptHash,
        repayment: decodedLoanReceipt.repayment + totalInterest,
      });
      ...
      expect(actualGracePeriodInterest).to.closeTo(totalInterest, "41000");
    });
  ...
  describe("#setGracePeriod", async function () {
    it("admin set grace period successfully", async function () {
      const pool_ = (await ethers.getContractAt(
        "WeightedRateCollectionPoolWithGracePeriod",
        await pool.getAddress()
      )) as WeightedRateCollectionPoolWithGracePeriod;
