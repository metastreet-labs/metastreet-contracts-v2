...
describe("BundleCollateralWrapper", function () {
  ...
  describe("#mint", async function () {
    it("mints bundle", async function () {
      ...
      const tokenId1 = (await extractEvent(mintTx1, bundleCollateralWrapper, "BundleCollateralWrapperTokenMinted")).args
        .tokenId;
      const tokenId2 = (await extractEvent(mintTx2, bundleCollateralWrapper, "BundleCollateralWrapperTokenMinted")).args
        .tokenId;
      ...
      await expectEvent(mintTx1, bundleCollateralWrapper, "BundleCollateralWrapperTokenMinted", {
        tokenId: tokenId1,
      ...
      await expectEvent(mintTx2, bundleCollateralWrapper, "BundleCollateralWrapperTokenMinted", {
        tokenId: tokenId2,
  ...
  describe("#enumerate", async function () {
    it("enumerate bundle", async function () {
      ...
      const assets = await bundleCollateralWrapper.enumerate(tokenId1, context);

      expect(assets[0].assetType).to.equal(0);
      expect(assets[0].token).to.equal(nft1.address);
      expect(assets[0].tokenId).to.equal(123);
      expect(assets[1].assetType).to.equal(0);
      expect(assets[1].token).to.equal(nft1.address);
      expect(assets[1].tokenId).to.equal(456);
      expect(assets[2].assetType).to.equal(0);
      expect(assets[2].token).to.equal(nft1.address);
      expect(assets[2].tokenId).to.equal(768);
    });
  ...
  describe("#unwrap", async function () {
    it("unwrap bundle", async function () {
      ...
      await expect(bundleCollateralWrapper.connect(accountBorrower).unwrap(tokenId1, context))
        ...
        .to.emit(bundleCollateralWrapper, "BundleCollateralWrapperTokenUnwrapped")
