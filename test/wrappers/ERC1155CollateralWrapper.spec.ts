...
describe("ERC1155CollateralWrapper", function () {
  ...
  describe("#size", async function () {
    it("size batch", async function () {
      ...
      /* Enumerate */
      const count = await ERC1155CollateralWrapper.size(tokenId1, context);
    ...
    it("fails on incorrect tokenId", async function () {
      ...
      await expect(ERC1155CollateralWrapper.size(badTokenId, context)).to.be.revertedWithCustomError(
        ERC1155CollateralWrapper,
        "InvalidContext"
      );
    });
        ...
        await ERC1155CollateralWrapper.supportsInterface(
          ethers.utils.hexlify(
            ethers.BigNumber.from(ERC1155CollateralWrapper.interface.getSighash("name"))
              ...
              .xor(ethers.BigNumber.from(ERC1155CollateralWrapper.interface.getSighash("size")))
          )
        )
      ).to.equal(true);
