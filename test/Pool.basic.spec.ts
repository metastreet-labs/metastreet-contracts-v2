...
describe("Pool Basic", function () {
  ...
  describe("constants", async function () {
    ...
    it("matches expected implementation version", async function () {
      expect(await pool.IMPLEMENTATION_VERSION()).to.equal("2.16");
    });
  });
  ...
  describe("getters", async function () {
    ...
    describe("#redemptionSharePrice", async function () {
      ...
      it("returns correct redemption share price after borrow", async function () {
        ...
        /* Deposit */
        const redemptionTx = await pool.connect(accountDepositors[0]).redeem(Tick.encode("10"), sharesMinted);
  ...
  describe("#deposit", async function () {
    ...
    it("successfully deposits at new tick after garbage collecting old tick", async function () {
      ...
      /* Redeem 1 shares */
      const redeemTx = await pool.connect(accountDepositors[0]).redeem(Tick.encode("10"), sharesMinted);
  ...
  describe("#redemptionAvailable", async function () {
    ...
    it("returns zero redemption available from dust cash", async function () {
      ...
      /* Redeem all shares */
      await pool.connect(accountDepositors[0]).redeem(Tick.encode("10"), sharesMinted);
  ...
  describe("#rebalance", async function () {
    it("rebalances a full redemption into another tick", async function () {
      ...
      const sharesMinted2 = (await pool.deposits(accountDepositors[0].address, Tick.encode("15"))).shares;
    ...
    it("fails on insufficient shares", async function () {
      ...
      /* Revert since shares received is 0 */
      await expect(
        pool.connect(accountDepositors[0]).rebalance(Tick.encode("1"), Tick.encode("2"), 0, 0)
      ).to.be.revertedWithCustomError(pool, "InsufficientShares");

      /* Redeem all shares */
      await pool.connect(accountDepositors[1]).redeem(Tick.encode("2"), sharesMinted2);

      /* Revert since shares received less than min shares */
      await expect(
        pool.connect(accountDepositors[1]).rebalance(Tick.encode("2"), Tick.encode("1"), 0, Tick.encode("1"))
      ).to.be.revertedWithCustomError(pool, "InsufficientShares");
    });
  ...
  describe("#repay", async function () {
    ...
    it("repays loan at maturity1", async function () {
    ...
    it("approved operator can repay", async function () {
      const [loanReceipt, loanReceiptHash] = await createActiveLoan(FixedPoint.from("25"));
      ...
      /* Set operator */
      await pool.connect(accountBorrower).setOperator(await accountLiquidator.getAddress(), true);

      /* Approve */
      await tok1.connect(accountLiquidator).approve(await pool.getAddress(), repayment);

      await helpers.time.increaseTo(decodedLoanReceipt.maturity + 1n);

      const repayTx = await pool.connect(accountLiquidator).repay(loanReceipt);
  ...
  describe("#refinance", async function () {
    ...
    it("approved operator refinance loan at maturity with admin fee and same principal", async function () {
      /* Set Admin Fee */
      await pool.setAdminFee(4400, accounts[2].address, 5000);
      ...
      /* Approve */
      await tok1.connect(accountLiquidator).approve(await pool.getAddress(), decodedLoanReceipt.repayment);
    ...
    it("vests lender surplus from liquidation to ticks", async function () {
      ...
      for (const nodeReceipt of decodedLoanReceipt.nodeReceipts) {
        ...
        const value =
          ...
          nodeReceipt.used +
          proceeds +
          (surplus / (86400n * 7n)) * (86400n * 3n + BigInt(i + 1));
        expect(node.value).to.equal(value);
        expect(node.available).to.equal(value);
        expect(node.pending).to.equal(0n);
        proceedsRemaining = proceedsRemaining - proceeds;
      ...
      /* Wait for vesting to full elapse */
      await helpers.time.increase(7n * 86400n);
      ...
      /* Validate ticks */
      i = 0;
      proceedsRemaining = lendersProceeds;
      for (const nodeReceipt of decodedLoanReceipt.nodeReceipts) {
        const node = await pool.liquidityNode(nodeReceipt.tick);
        const surplus = ((nodeReceipt.pending - nodeReceipt.used) * lendersSurplus) / totalInterest;
        const proceeds =
          i == decodedLoanReceipt.nodeReceipts.length - 1
            ? proceedsRemaining
            : (lendersProceeds * nodeReceipt.pending) / totalPending;
        const value = FixedPoint.from("25") + FixedPoint.from("2") - nodeReceipt.used + proceeds + surplus;
        expect(node.value).to.equal(value);
        expect(node.available).to.equal(value);
        expect(node.pending).to.equal(0n);
        proceedsRemaining = proceedsRemaining - proceeds;
