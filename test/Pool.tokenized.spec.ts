...
describe("Pool Tokenized", function () {
  ...
  const _computeDeterministicAddress = async (tick: BigNumber) => {
    ...
    const beaconProxyBytecode = (await ethers.getContractFactory("DepositERC20Proxy")).bytecode;
  ...
  describe("DepositERC20Factory", async function () {
    ...
    describe("#implementation", async function () {
      it("returns correct implementation address", async function () {
        ...
        expect(tokenImplementation).to.equal(await pool.getDepositERC20Implementation());
        expect(depositERC20Impl.address).to.equal(await pool.getDepositERC20Implementation());
      });
    });
