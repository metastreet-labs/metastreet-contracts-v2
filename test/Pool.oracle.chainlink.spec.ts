...
describe("Pool Chainlink Price Oracle", function () {
  ...
  /* ETH/USD */
  const QUOTE_PRICE_FEED = "0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419";
  ...
  before("deploy fixture", async function () {
    ...
    await tok1.connect(accountDepositor).transfer(await accountLiquidator.getAddress(), ethers.parseEther("1"));
  ...
  async function sourceLiquidity(
    ...
    const oraclePrice = await priceOracle.price(XAUM_ADDRESS, WETH_ADDRESS, [XAUM_ID], [1], oracleContext);
  ...
  describe("#quote", async function () {
    ...
    it("correctly quotes repayment for single collateral", async function () {
      ...
      ).to.equal(FixedPoint.from("10.082191780812159996"));
      ...
      ).to.equal(FixedPoint.from("25.205479452030399991"));
    });
  });
  ...
  describe("#borrow", async function () {
    ...
    it("originates loan", async function () {
      ...
      expect(decodedLoanReceipt.nodeReceipts.length).to.equal(22);
  ...
  describe("#refinance", async function () {
    ...
    it("refinance loan with same principal and oracle context", async function () {
      ...
      expect(decodedNewLoanReceipt.collateralTokenId).to.equal(XAUM_ID);
      expect(decodedNewLoanReceipt.nodeReceipts.length).to.equal(22);
    });
  });
});
