...
describe.only("Pool Bundle", function () {
  ...
  before("deploy fixture", async () => {
    ...
    await dn404MirrorNFT.connect(accountBorrower).setApprovalForAll(bundleCollateralWrapper.address, true);
  ...
  async function createActiveBundleLoan(
    ...
    /* Mint bundle */
    const mintTx = await bundleCollateralWrapper.connect(accountBorrower).mint(dn404MirrorNFT.address, [1, 2, 3]);
  ...
  describe("#repay", async function () {
    ...
    it("repays bundle loan at maturity", async function () {
      const mintTx = await bundleCollateralWrapper.connect(accountBorrower).mint(dn404MirrorNFT.address, [2, 3]);
      const bundleTokenId = (await extractEvent(mintTx, bundleCollateralWrapper, "BundleMinted")).args.tokenId;
      const bundleData = (await extractEvent(mintTx, bundleCollateralWrapper, "BundleMinted")).args.encodedBundle;
