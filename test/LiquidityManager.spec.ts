...
describe("LiquidityManager", function () {
  ...
  describe("#instantiate", async function () {
    it("instantiates a new liquidity node", async function () {
      ...
      await liquidityManager.instantiate(FixedPoint.from("1"));

      /* Validate nodes */
      let nodes = await liquidityManager.liquidityNodes(ethers.constants.Zero, MaxUint128);
      expect(nodes.length).to.equal(2);
      expect(nodes[0].prev).to.equal(0);
      expect(nodes[0].next).to.equal(FixedPoint.from("1"));
      expect(nodes[1].prev).to.equal(0);
      expect(nodes[1].next).to.equal(MaxUint128);

      /* Instantiate two additional nodes */
      await liquidityManager.instantiate(FixedPoint.from("10"));
      await liquidityManager.instantiate(FixedPoint.from("50"));
      ...
      expect(nodes[0].next).to.equal(FixedPoint.from("1"));
      expect(nodes[1].prev).to.equal(0);
      expect(nodes[1].next).to.equal(FixedPoint.from("10"));
      expect(nodes[2].prev).to.equal(FixedPoint.from("1"));
      expect(nodes[2].next).to.equal(FixedPoint.from("50"));
      expect(nodes[3].prev).to.equal(FixedPoint.from("10"));
    ...
    it("fails on insufficient tick spacing", async function () {
      ...
      await expect(liquidityManager.instantiate(FixedPoint.from("1.05"))).to.be.revertedWithCustomError(
        liquidityManager,
        "InsufficientTickSpacing"
      );
      /* Try to instantiate another node that is 5% lower */
      await expect(liquidityManager.instantiate(FixedPoint.from("0.95"))).to.be.revertedWithCustomError(
        liquidityManager,
        "InsufficientTickSpacing"
      );
    });
    it("fails on insolvent node", async function () {
      ...
      await liquidityManager.instantiate(FixedPoint.from("3"));
      await liquidityManager.deposit(FixedPoint.from("3"), FixedPoint.from("5"));
      await liquidityManager.use(FixedPoint.from("3"), FixedPoint.from("5"), FixedPoint.from("6"));
      ...
      await expect(liquidityManager.instantiate(FixedPoint.from("3"))).to.be.revertedWithCustomError(
        liquidityManager,
        "InsolventLiquidity"
      );
    });
  ...
  describe("#deposit", async function () {
    it("deposits into active node", async function () {
      ...
      /* Deposit */
      const depositTx = await liquidityManager.deposit(FixedPoint.from("3"), FixedPoint.from("5"));
      ...
      /* Validate node */
      const node = await liquidityManager.liquidityNode(FixedPoint.from("3"));
    ...
    it("deposits into active node that has appreciated", async function () {
      ...
      /* Deposit 2 */
      const depositTx = await liquidityManager.deposit(FixedPoint.from("3"), FixedPoint.from("3"));
    ...
    it("deposits into active node that has depreciated", async function () {
      ...
      /* Deposit 2 */
      const depositTx = await liquidityManager.deposit(FixedPoint.from("3"), FixedPoint.from("2"));
    ...
    it("deposits into active node that has pending returns", async function () {
      ...
      await liquidityManager.instantiate(FixedPoint.from("3"));
      await liquidityManager.deposit(FixedPoint.from("3"), FixedPoint.from("5"));
      await liquidityManager.use(FixedPoint.from("3"), FixedPoint.from("5"), FixedPoint.from("7"));
    ...
    it("fails on inactive node", async function () {
      await expect(liquidityManager.deposit(FixedPoint.from("3"), FixedPoint.from("5"))).to.be.revertedWithCustomError(
        liquidityManager,
        "InactiveLiquidity"
      );
    });
    it("fails on insolvent node", async function () {
      ...
      await expect(liquidityManager.deposit(FixedPoint.from("3"), FixedPoint.from("1"))).to.be.revertedWithCustomError(
        liquidityManager,
        "InactiveLiquidity"
      );
    });
  ...
  describe("#use", async function () {
    it("uses from active node", async function () {
      ...
      /* Use from node */
      await liquidityManager.use(FixedPoint.from("3"), FixedPoint.from("3"), FixedPoint.from("3.2"));
  ...
  describe("#restore", async function () {
    ...
    it("restores pending amount", async function () {
      /* Use */
      await liquidityManager.use(FixedPoint.from("3"), FixedPoint.from("3"), FixedPoint.from("4"));
      /* Restore */
      await liquidityManager.restore(
        FixedPoint.from("3"),
        FixedPoint.from("3"),
    ...
    it("restores less than pending and becomes insolvent", async function () {
      ...
      /* Validate node */
      let node = await liquidityManager.liquidityNode(FixedPoint.from("3"));
    ...
    it("restores less than pending and becomes insolvent dust", async function () {
      ...
      await liquidityManager.use(FixedPoint.from("3"), FixedPoint.from("5"), FixedPoint.from("6"));
      /* Restore 4 wei */
      await liquidityManager.restore(FixedPoint.from("3"), FixedPoint.from("5"), FixedPoint.from("6"), 4);
  ...
  describe("#redeem", async function () {
    ...
    it("redeems from current index", async function () {
      /* Redeem */
      const redeemTx1 = await liquidityManager.redeem(FixedPoint.from("3"), FixedPoint.from("1"));
      ...
      /* Redeem */
      const redeemTx2 = await liquidityManager.redeem(FixedPoint.from("3"), FixedPoint.from("1"));
    ...
    it("redeems from subsequent index", async function () {
      ...
      /* Process redemption */
      const processRedemptionTx = await liquidityManager.processRedemptions(FixedPoint.from("3"));
      ...
      /* Validate node */
      node = await liquidityManager.liquidityNode(FixedPoint.from("3"));
      expect(node.value).to.equal(FixedPoint.from("4"));
      expect(node.shares).to.equal(FixedPoint.from("4"));
      expect(node.available).to.equal(FixedPoint.from("4"));
      expect(node.pending).to.equal(ethers.constants.Zero);
  ...
  describe("#processRedemptions", async function () {
    ...
    it("processes redemption from available liquidity", async function () {
      /* Redeem twice */
      const redeemTx1 = await liquidityManager.redeem(FixedPoint.from("3"), FixedPoint.from("2"));
      const redeemTx2 = await liquidityManager.redeem(FixedPoint.from("3"), FixedPoint.from("1"));
      ...
      /* Process redemptions */
      const processRedemptionsTx = await liquidityManager.processRedemptions(FixedPoint.from("3"));
    ...
    it("processes redemption from restored liquidity", async function () {
      ...
      /* Redeem */
      const redeemTx = await liquidityManager.redeem(FixedPoint.from("3"), FixedPoint.from("2"));
    ...
    it("processes redemption from restored liquidity at multiple prices", async function () {
      /* Use */
      await liquidityManager.use(FixedPoint.from("3"), FixedPoint.from("2"), FixedPoint.from("3"));
      await liquidityManager.use(FixedPoint.from("3"), FixedPoint.from("3"), FixedPoint.from("4"));

      /* Redeem */
      const redeemTx = await liquidityManager.redeem(FixedPoint.from("3"), FixedPoint.from("3"));
      const [redemptionIndex, redemptionTarget] = (await extractEvent(redeemTx, liquidityManager, "RedemptionTarget"))
        .args;
    ...
    it("processes redemption from insolvent liquidity", async function () {
      ...
      /* Redeem 3 shares */
      const redeemTx1 = await liquidityManager.redeem(FixedPoint.from("3"), FixedPoint.from("3"));
      const [redemptionIndex1, redemptionTarget1] = (
        await extractEvent(redeemTx1, liquidityManager, "RedemptionTarget")
      ).args;
      ...
      const redeemTx2 = await liquidityManager.redeem(FixedPoint.from("3"), FixedPoint.from("2"));
      const [redemptionIndex2, redemptionTarget2] = (
        await extractEvent(redeemTx2, liquidityManager, "RedemptionTarget")
      ).args;
      ...
      /* Instantiate and deposit into node */
      await liquidityManager.instantiate(FixedPoint.from("3"));
      await liquidityManager.deposit(FixedPoint.from("3"), FixedPoint.from("4"));
  ...
  async function setupLiquidity(): Promise<void> {
    ...
    for (const depth of [FixedPoint.from("10"), FixedPoint.from("20"), FixedPoint.from("30"), FixedPoint.from("40")]) {
      await liquidityManager.instantiate(depth);
      await liquidityManager.deposit(depth, FixedPoint.from("50"));
    }

    /* Setup insolvent liquidity at 50 ETH */
    await liquidityManager.instantiate(FixedPoint.from("50"));
    await liquidityManager.deposit(FixedPoint.from("50"), FixedPoint.from("5"));
    await liquidityManager.use(FixedPoint.from("50"), FixedPoint.from("5"), FixedPoint.from("6"));
    await liquidityManager.restore(
      FixedPoint.from("50"),
      FixedPoint.from("5"),
      FixedPoint.from("6"),
  ...
  describe("#source", async function () {
    const depths = [FixedPoint.from("10"), FixedPoint.from("20"), FixedPoint.from("30"), FixedPoint.from("40")];

    beforeEach("setup liquidity", async function () {
      await setupLiquidity();
    });
    it("sources required liquidity with 1 token", async function () {
      let [nodes, count] = await liquidityManager.source(FixedPoint.from("15"), depths, 1);

      /* Validate nodes */
      expect(count).to.equal(2);
      expect(nodes[0].depth).to.equal(FixedPoint.from("10"));
      expect(nodes[0].available).to.equal(FixedPoint.from("40"));
      expect(nodes[0].used).to.equal(FixedPoint.from("10"));
      expect(nodes[1].depth).to.equal(FixedPoint.from("20"));
      expect(nodes[1].available).to.equal(FixedPoint.from("45"));
      expect(nodes[1].used).to.equal(FixedPoint.from("5"));

      [nodes, count] = await liquidityManager.source(FixedPoint.from("35"), depths, 1);
    ...
    it("sources required liquidity with 3 tokens", async function () {
      let [nodes, count] = await liquidityManager.source(FixedPoint.from("15"), depths, 3);
      ...
      [nodes, count] = await liquidityManager.source(FixedPoint.from("35"), depths, 3);
      ...
      [nodes, count] = await liquidityManager.source(FixedPoint.from("120"), depths, 3);
    ...
    it("sources required liquidity with 10 tokens", async function () {
      let [nodes, count] = await liquidityManager.source(FixedPoint.from("15"), depths, 10);
      ...
      [nodes, count] = await liquidityManager.source(FixedPoint.from("35"), depths, 10);
      ...
      [nodes, count] = await liquidityManager.source(FixedPoint.from("120"), depths, 10);
      ...
      [nodes, count] = await liquidityManager.source(FixedPoint.from("200"), depths, 10);
    ...
    it("fails on insufficient liquidity", async function () {
      await expect(liquidityManager.source(FixedPoint.from("25"), depths.slice(0, 2), 1)).to.be.revertedWithCustomError(
        liquidityManager,
        "InsufficientLiquidity"
      );
      await expect(liquidityManager.source(FixedPoint.from("45"), depths, 1)).to.be.revertedWithCustomError(
        liquidityManager,
        "InsufficientLiquidity"
      );
      await expect(liquidityManager.source(FixedPoint.from("121"), depths, 3)).to.be.revertedWithCustomError(
        liquidityManager,
        "InsufficientLiquidity"
      );
      await expect(liquidityManager.source(FixedPoint.from("201"), depths, 10)).to.be.revertedWithCustomError(
        liquidityManager,
        "InsufficientLiquidity"
      );
    });
    it("fails on non-increasing depths", async function () {
      await expect(
        liquidityManager.source(
          FixedPoint.from("35"),
          [FixedPoint.from("10"), FixedPoint.from("30"), FixedPoint.from("20"), FixedPoint.from("40")],
          1
        )
    ...
    it("fails on duplicate depths", async function () {
      await expect(
        liquidityManager.source(
          FixedPoint.from("25"),
          [FixedPoint.from("10"), FixedPoint.from("20"), FixedPoint.from("20"), FixedPoint.from("40")],
          1
        )
      ).to.be.revertedWithCustomError(liquidityManager, "InvalidDepths");
    });
  ...
  describe("#utilization", async function () {
    ...
    it("returns utilization", async function () {
      ...
      await liquidityManager.use(FixedPoint.from("10"), FixedPoint.from("10"), FixedPoint.from("11"));
      await liquidityManager.use(FixedPoint.from("20"), FixedPoint.from("15"), FixedPoint.from("16"));

      /* Check utilization after usage */
      expect(await liquidityManager.utilization()).to.equal(FixedPoint.from("0.125"));

      /* Deposit */
      await liquidityManager.deposit(FixedPoint.from("30"), FixedPoint.from("50"));
  ...
  describe("#liquidityAvailable", async function () {
    ...
    it("returns liquidity available", async function () {
      ...
      expect(await liquidityManager.liquidityAvailable(MaxUint128, 1)).to.equal(FixedPoint.from("40"));
      expect(await liquidityManager.liquidityAvailable(MaxUint128, 5)).to.equal(FixedPoint.from("200"));

      /* Use from highest node (artifical) */
      await liquidityManager.use(FixedPoint.from("40"), FixedPoint.from("45"), FixedPoint.from("46"));

      /* Check liquidity available after usage */
      expect(await liquidityManager.liquidityAvailable(MaxUint128, 1)).to.equal(FixedPoint.from("35"));

      /* Deposit into lowest and highest nodes */
      await liquidityManager.deposit(FixedPoint.from("10"), FixedPoint.from("2"));
      await liquidityManager.deposit(FixedPoint.from("40"), FixedPoint.from("2"));

      /* Check liquidity available after deposit */
      expect(await liquidityManager.liquidityAvailable(MaxUint128, 1)).to.equal(FixedPoint.from("37"));

      /* Check liquidity available at lower tiers */
      expect(await liquidityManager.liquidityAvailable(FixedPoint.from("20"), 1)).to.equal(FixedPoint.from("20"));
      expect(await liquidityManager.liquidityAvailable(FixedPoint.from("10"), 1)).to.equal(FixedPoint.from("10"));

      /* Check liquidity available at lower tiers with 3 items */
      expect(await liquidityManager.liquidityAvailable(FixedPoint.from("10"), 3)).to.equal(FixedPoint.from("30"));
      expect(await liquidityManager.liquidityAvailable(FixedPoint.from("20"), 3)).to.equal(FixedPoint.from("60"));

      /* Use nearly all of the lowest node (leaving 2 ETH amount) */
      await liquidityManager.use(FixedPoint.from("10"), FixedPoint.from("50"), FixedPoint.from("51"));

      /* Check liquidity available at lower tiers */
      expect(await liquidityManager.liquidityAvailable(FixedPoint.from("10"), 1)).to.equal(FixedPoint.from("2"));
      expect(await liquidityManager.liquidityAvailable(FixedPoint.from("20"), 1)).to.equal(FixedPoint.from("20"));

      /* Check liquidity available at lower tiers with 3 items */
      expect(await liquidityManager.liquidityAvailable(FixedPoint.from("10"), 3)).to.equal(FixedPoint.from("2"));
      expect(await liquidityManager.liquidityAvailable(FixedPoint.from("20"), 3)).to.equal(FixedPoint.from("52"));
    });
  });
});
