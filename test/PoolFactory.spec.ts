...
describe("PoolFactory", function () {
  ...
  describe("#setAdminFeeRate", async function () {
    ...
    it("fails on invalid pool address", async function () {
      ...
      await expect(
        poolFactory.adminCall(await poolFactory.getAddress(), data)
    ...
    it("fails on invalid caller", async function () {
      const data = iface.encodeFunctionData("setAdminFee", [700, ethers.ZeroAddress, 0]);
      await expect(
        poolFactory.connect(accounts[1]).adminCall(await pool2.getAddress(), data)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
    ...
    it("fails on invalid rate", async function () {
      ...
      await expect(
        poolFactory.adminCall(await pool2.getAddress(), data1)
      ...
      await expect(
        poolFactory.adminCall(await pool2.getAddress(), data2)
      ).to.be.revertedWithCustomError(pool2, "InvalidParameters");
    });
  });
  ...
  describe("#withdrawAdminFees", async function () {
    ...
    it("fails on invalid caller", async function () {
      const data = iface.encodeFunctionData("withdrawAdminFees", [accounts[2].address]);
      await expect(
        poolFactory.connect(accounts[1]).adminCall(await pool1.getAddress(), data)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
  });
