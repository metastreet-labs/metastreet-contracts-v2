...
describe("Pool ERC1155", function () {
  ...
  before("deploy fixture", async () => {
    ...
    await nft1.mintBatch(accountBorrower.address, [123, 124, 125], [1, 1, 1], "0x");
  ...
  async function createActiveLoan(
    ...
    const tokenId = (await nft1.balanceOf(accountBorrower.address, 123)).eq(ethers.BigNumber.from("1"))
      ? 123
      : (await nft1.balanceOf(accountBorrower.address, 124)).eq(ethers.BigNumber.from("1"))
      ? 124
      : 125;
  ...
  describe("#liquidate", async function () {
    ...
    it("liquidates expired loan", async function () {
      ...
      /* Validate events */
      await expectEvent(liquidateTx, nft1, "TransferSingle", {
        from: pool.address,
