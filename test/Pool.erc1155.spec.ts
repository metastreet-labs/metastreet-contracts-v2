...
describe("Pool ERC1155", function () {
  ...
  before("deploy fixture", async () => {
    ...
    const poolImplFactory = await getContractFactoryWithLibraries("WeightedRateERC1155CollectionPool", [
      "LiquidityLogic",
      "DepositLogic",
      "BorrowLogic",
    ...
    await nft1.mintBatch(await accountBorrower.getAddress(), [123, 124, 125], [1, 1, 1], "0x");
  ...
  describe("constants", async function () {
    ...
    it("matches expected implementation version", async function () {
      expect(await pool.IMPLEMENTATION_VERSION()).to.equal("2.13");
    });
  });
  ...
  describe("#redeem", async function () {
    it("successfully redeems entire deposit from available cash", async function () {
      ...
      expect(redemptionId).to.equal(1n);
    ...
    it("successfully schedules multiple redemptions", async function () {
      ...
      expect(redemptionId).to.equal(2n);
  ...
  describe("#redemptionAvailable", async function () {
    ...
    it("returns written down redemption available from liquidated loan", async function () {
      ...
      /* Get minted info */
      const collateralWrapperContext = (await extractEvent(liquidateTx, erc1155CollateralWrapper, "BatchMinted")).args
        .encodedBatch;
      const tokenId = (await extractEvent(liquidateTx, erc1155CollateralWrapper, "BatchMinted")).args.tokenId;
    ...
    it("returns zero redemption available from dust cash", async function () {
      ...
      expect(redemption.index).to.equal(1n);
  ...
  describe("#onCollateralLiquidated", async function () {
    ...
    it("processes liquidated loan for higher proceeds with lender surplus but no borrower surplus", async function () {
      ...
      /* Liquidate collateral and process liquidation */
      const proceeds = 25203000000000000000n;
      const onCollateralLiquidatedTx = await collateralLiquidator
        .connect(accountLiquidator)
        .liquidateCollateral(
          await pool.getAddress(),
