...
describe("Pool Ranged Collection", function () {
  ...
  before("deploy fixture", async () => {
    ...
    const poolImplFactory = await ethers.getContractFactory("WeightedRateRangedCollectionPoolV2");
  ...
  describe("constants", async function () {
    it("matches expected implementation name", async function () {
      expect(await pool.IMPLEMENTATION_NAME()).to.equal("WeightedRateRangedCollectionPoolV2");
    });
  });
  ...
  describe("#quote", async function () {
    ...
    it("correctly quotes repayment with ranged collection filter", async function () {
      expect(
        await pool.quote(
          FixedPoint.from("10"),
          30 * 86400,
  ...
  describe("#borrow", async function () {
    ...
    it("originates loan with ranged collection filter", async function () {
      /* Quote repayment */
    ...
    it("originates loan with delegation with ranged collection filter", async function () {
  ...
  describe("#refinance", async function () {
    ...
    it("refinance loan at maturity with same principal and ranged collection filter", async function () {
      const borrowTx = await pool
        .connect(accountBorrower)
        .borrow(
          FixedPoint.from("25"),
